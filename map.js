...
var ind_state = new ol.layer.Image({
            ...
            source: new ol.source.ImageWMS({
                ...
                params: {
                    'LAYERS': 'r8gcp:leyte_gcp'
                },
                ratio: 1,
                serverType: 'geoserver'
            })
        });
var r8_shp = new ol.layer.Image({
            title: 'r8_shp',
            // extent: [-180, -90, -180, 90],
            source: new ol.source.ImageWMS({
                url: 'http://localhost:8084/geoserver/wms',
...
overlays.getLayers().push(r8_shp);
...
function legend() {
    ...
    for (var i = 0; i < no_layers; i++) {
        ar.push(
            "http://localhost:8084/geoserver/r8gcp/wms?REQUEST=GetLegendGraphic&VERSION=1.0.0&FORMAT=image/png&WIDTH=20&HEIGHT=20&LAYER=" +
...
map.getLayers().on("change", legend);

// Toggle Legend Button Functionality
document.getElementById("legend_toggle").onclick = function() {
    var legendContainer = document.getElementById("legend");
...
// Add LayerSwitcher with Legend Integration
layerSwitcher = new ol.control.LayerSwitcher({
    activationMode: 'click',
    startActive: true,
    tipLabel: 'Layers', // Optional label for button
...
$(document).ready(function() {
    $.ajax({
        type: "GET",
        url: "http://localhost:8084/geoserver/r8gcp/wfs?request=getCapabilities",
        dataType: "xml",
...
$(function() {
    $("#layer").change(function() {
        ...
        $(document).ready(function() {
            $.ajax({
                type: "GET",
                url: "http://localhost:8084/geoserver/r8gcp/wfs?service=WFS&request=DescribeFeatureType&version=1.1.0&typeName=" + value_layer,
                dataType: "xml",
                success: function(xml) {
...
$(document).ready(function() {
    $.ajax({
        type: "GET",
        url: "http://localhost:8084/r8gcp/geoserver/wfs?request=getCapabilities",
        dataType: "xml",
        success: function(xml) {
            var select = $('#layer1');
            $(xml).find('FeatureType').each(function() {
...
function wms_layers() {
    ...
    $(document).ready(function() {
        $.ajax({
            type: "GET",
            url: "http://localhost:8084/geoserver/r8gcp/wms?request-getCapabilities",
            dataType: "xml",
            success: function(xml) {
